...
-- Chisel description
description = "Show the top process defined by the highest CPU utilization. This chisel is compatible with slurm with cgroups using the sysdig -pc or -pcontainer argument, otherwise no container information will be shown."
short_description = "Top processes by CPU usage with slurm cgroups"
category = "CPU Usage"
...
function on_init()
	...
	if print_container then
		...
		vizinfo.key_fld = {"evt.cpu", "proc.name", "proc.pid", "proc.vpid", "proc.vmsize_kb", "thread.cgroup.cpuset", "proc.exe", "proc.args", "proc.ppid"}
		vizinfo.key_desc = {"CPU nb", "Process", "Host_pid", "Container_pid", "VM size", "container.id", "exe", "args"," Process parent id"}
		vizinfofd.key_fld = {"proc.pid", "proc.vpid", "thread.cgroup.cpuset", "fd.name"}
		vizinfofd.key_desc = {"Host_pid", "Container_pid", "container.id", "File name"}
    ...
    fcgroup = chisel.request_field("thread.cgroup.cpuset")
...
function on_event()
	local key = nil
	local kv = nil
	local containerName = evt.field(fcgroup)
    local procname = evt.field(fpname)


	if containerName == '' or procname == 'sysdig' then
	 	 return true
        end
